...
const DEFAULT_DATA = [
  // Extracted from original: 'New Microsoft Excel Worksheet.xlsx'
...
let state = { data: DEFAULT_DATA, src: 'New Microsoft Excel Worksheet.xlsx' };
let charts = {};
...
document.getElementById('fileUpload').addEventListener('change', async function(e) {
  ...
  try {
    ...
    state.src = file.name;
    init();
    toast('✅ Dashboard updated from ' + file.name);

  } catch (err) {
    console.error(err);
    toast('❌ Error reading Excel: ' + err.message);
  }
  e.target.value = '';
});
